...
from tyler_functions import get_date_time
...
base_dir = '.../modelsAEGeAN1024_{}_{}/'.format(dataroot.split('/')[-1], date)  # Useful for making many models
...
optim_discriminator = optim.SGD(_Discriminator.parameters(), lr=2e-4, momentum=0.9)
...
for epoch in range(n_iter):
    for i, (batch, _) in enumerate(dataloader):
        ...
        if i % 35 == 0:
            real_grid = vutils.make_grid(batch, n_image_rows, 0, normalize=True)
            real_grid = to_image(real_grid)
            real_grid.save('{}/real_samples.gif'.format(save_dir))

            decoded_grid = vutils.make_grid(reconstructed.data.clone().cpu(), n_image_rows, 0, normalize=True)
            decoded_grid = to_image(decoded_grid)
            decoded_grid.save('{}/decoded_samples_epoch_{}_{}.gif'.format(save_dir, epoch, i))

            fake = _Decoder(fixed_noise, mode='gen')
            fake_grid = vutils.make_grid(fake.data.clone().cpu(), n_image_rows, 0, normalize=True)
            fake_grid = to_image(fake_grid)
            fake_grid.save('{}/fake_samples_epoch_{}_{}.gif'.format(save_dir, epoch, i))

    if epoch % (n_iter // num_checkpoints) == 0 and epoch is not 0 or epoch in save_list:  # save n checkpoints (with optional list)
        torch.save(_Encoder.state_dict(), '{}Encoder_nf{}_e{}_recon-err{:.4f}.pth'.format(checkpoint_path, nf, epoch, reconstruction_loss.data[0]))
        torch.save(_Decoder.state_dict(), '{}Decoder_nf{}_e{}_fake-err{:.4f}.pth'.format(checkpoint_path, nf, epoch, fake_samples_loss.data[0]))
        torch.save(_Discriminator.state_dict(), '{}Discriminator_nf{}_e{}_discrim-err{:.4f}.pth'.format(checkpoint_path, nf, epoch, discriminator_total_loss.data[0]))
